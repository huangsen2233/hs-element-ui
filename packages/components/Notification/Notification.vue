...
import { useOffset } from "@hs-element-ui/hooks";
import { addUnit } from "@hs-element-ui/utils";
import { typeIconMap, RenderVnode } from "@hs-element-ui/utils";
import HsIcon from "../Icon/Icon.vue";

defineOptions({ name: "HsNotification" });
...
<template>
    ...
    >
        ...
        >
            <hs-icon v-if="iconName" :icon="iconName" class="er-notification__icon" />
            ...
            <div class="er-notification__close" v-if="showClose">
                <hs-icon icon="xmark" @click.stop="close" />
            </div>
        </div>
    </transition>
</template>
