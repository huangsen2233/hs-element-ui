...
调用 `HsMessageBox.alert` 方法以打开 alert 框。
...
调用 `HsMessageBox.confirm` 方法以打开 confirm 框。
...
调用 `HsMessageBox.prompt` 方法以打开 prompt 框。
...
可以通过配置 `HsMessageBox` 的参数来实现一些个性化需求（options 参数见下文）。
...
`HsMessageBox` 提供了全局方法 `HsMessageBox.alert`、`HsMessageBox.confirm`、`HsMessageBox.prompt`，用于在需要时弹出消息弹框。
